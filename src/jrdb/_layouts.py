...
# SED 成績データ（第4版a・2022.08.27）。レコード長376。出走馬単位 (race_id, 馬番)。
# 過去走参照（血統登録番号+年月日）＋JRDB上書き統合の raw_results 供給源。
# 着順/タイム/斤量/騎手/調教師/払戻/賞金/コーナー順位＝netkeiba raw_results 相当。
SED = {
    "race_key": (1, 8),
    "umaban": (9, 2),
    "ketto": (11, 8),
    "ymd": (19, 8),          # YYYYMMDD（過去走の日付）
    "bamei": (27, 36),       # 馬名（全角18文字）
    # ── レース条件（BAC 相当。BAC 未取得年の raw_race_info 代替にも）──
    "kyori": (63, 4),        # 距離
    "shiba_dirt": (67, 1),   # 芝ダ障害 1芝/2ダート/3障害
    "migi_hidari": (68, 1),  # 右左
    "uchi_soto": (69, 1),    # 内外
    "baba_state": (70, 2),   # 馬場状態（コード）
    "shubetsu": (72, 2),     # 種別（コード）
    "joken": (74, 2),        # 条件（英字含みうる）
    "kigou": (76, 3),        # 記号（コード）
    "juryo_code": (79, 1),   # 重量（ハンデ等コード）
    "grade": (80, 1),        # グレード
    "race_name": (81, 50),   # レース名（全角25文字）
    "toushuu": (131, 2),     # 頭数
    "race_name_short": (133, 8),  # レース名略称
    # ── 馬成績 ──
    "chakujun": (141, 2),    # 着順
    "ijo_kubun": (143, 1),   # 異常区分（取消/除外/中止等）
    "time": (144, 4),        # タイム（1byte分+3byte秒0.1・文字列保持）
    "futan_juryo": (148, 3),  # 斤量 0.1kg
    "kishu_name": (151, 12),  # 騎手名（全角6文字）
    "chokyoshi_name": (163, 12),  # 調教師名
    "kakutei_tansho": (175, 6),   # 確定単勝オッズ
    "kakutei_ninki": (181, 2),    # 確定単勝人気順位
    # ── JRDBデータ（指数・展開）──
    "idm": (183, 3),
    "soten": (186, 3),       # 素点
    "babasa": (189, 3),      # 馬場差
    ...
    "naka_furi": (207, 3),   # 中不利
    "ato_furi": (210, 3),    # 後不利
    "race_furi": (213, 3),   # レース（不利集約）
    "course_dori": (216, 1),  # コース取り 1最内..5大外
    "joushoudo_code": (217, 1),  # 上昇度コード
    "class_code": (218, 2),  # クラスコード
    "batai_code": (220, 1),  # 馬体コード
    "kehai_code": (221, 1),  # 気配コード
    "race_pace": (222, 1),   # レースペース H/M/S
    "uma_pace": (223, 1),    # 馬ペース H/M/S
    "ten_idx": (224, 5),     # テン指数
    "agari_idx": (229, 5),   # 上がり指数
    "pace_idx": (234, 5),    # ペース指数
    "race_p_idx": (239, 5),  # レースP指数
    "chaku1_bamei": (244, 12),   # 1(2)着馬名
    "chaku1_time_sa": (256, 3),  # 1(2)着タイム差 0.1秒
    "mae3f_time": (259, 3),  # 前3Fタイム 0.1秒
    "ato3f_time": (262, 3),  # 後3Fタイム 0.1秒
    "biko": (265, 24),       # 備考（地方競馬場名等）
    # ── 第2版追加（早期オッズ・コーナー順位・リンクコード）──
    # 10時単勝オッズ = 発走前朝10時の単勝＝歴史的な「早期市場」（早期オッズ路線の核）。
    "kakutei_fukusho_shita": (291, 6),  # 確定複勝オッズ下
    "odds_10_tansho": (297, 6),         # 10時単勝オッズ（早期市場）
    "odds_10_fukusho": (303, 6),        # 10時複勝オッズ
    "corner1": (309, 2),     # コーナー順位1（=通過順）
    "corner2": (311, 2),
    "corner3": (313, 2),
    "corner4": (315, 2),
    "mae3f_sa": (317, 3),    # 前3F先頭差 0.1秒
    "ato3f_sa": (320, 3),    # 後3F先頭差 0.1秒
    "kishu_code": (323, 5),  # 騎手マスタ(KSA)リンク
    "chokyo_code": (328, 5),  # 調教師マスタ(CSA)リンク
    # ── 第3版追加（馬体重・天候・脚質）──
    "bataijuu": (333, 3),    # 馬体重
    "bataijuu_zougen": (336, 3),  # 馬体重増減（符号+2桁・文字列）
    "tenko_code": (339, 1),  # 天候コード
    "course": (340, 1),      # コース 1A..6D
    "race_kyakushitsu": (341, 1),  # レース脚質
    # ── 第4版a追加（払戻・賞金・ペース流れ・発走時刻）──
    "tansho_payoff": (342, 7),    # 単勝払戻（円）
    "fukusho_payoff": (349, 7),   # 複勝払戻（円）
    "honshokin": (356, 5),        # 本賞金（万円）
    "shutokushokin": (361, 5),    # 収得賞金（万円）
    "race_pace_nagare": (366, 2),  # レースペース流れ（コード）
    "uma_pace_nagare": (368, 2),   # 馬ペース流れ（コード）
    "yonkaku_course_dori": (370, 1),  # 4角コース取り 1最内..5大外
    "hassou_time": (371, 4),      # 発走時間 HHMM
}
