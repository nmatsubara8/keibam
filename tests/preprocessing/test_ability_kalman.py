...
def test_datamerger_merge_ability_adds_columns():
    ...
    obj._merged_data = md
    obj.horse_ability_kf_snapshot = {}
    obj._merge_horse_ability_kalman()
    for col in KF_FEATURE_COLS:
        assert col in obj._merged_data.columns
    assert "A" in obj.horse_ability_kf_snapshot


def test_shutuba_merger_live_ability_from_snapshot(tmp_path, monkeypatch):
    import json
    ...
    snap = {
        "A": {"level": 3.0, "trend": 0.2, "var_level": 0.5, "workload": 2.0,
              "n_races": 10, "last_date": "2020-01-01"},
        "B": {"level": -1.0, "trend": 0.0, "var_level": 0.5, "workload": 1.0,
              "n_races": 10, "last_date": "2020-01-01"},
    }
    snap_path = tmp_path / "horse_ability_kf.json"
    snap_path.write_text(json.dumps(snap))
    monkeypatch.setattr(_local_paths.LocalPaths, "HORSE_ABILITY_KF_PATH", str(snap_path))

    md = pd.DataFrame(
        {"horse_id": ["A", "B", "C"], "馬番": [1, 2, 3], "date": ["2020-02-01"] * 3},
    ...
    # A: kf_level = level + trend = 3.2
    assert out.loc[out["horse_id"] == "A", "kf_level"].iloc[0] == pytest.approx(3.2)
    # 未知の馬 C は prior
    assert out.loc[out["horse_id"] == "C", "kf_level"].iloc[0] == pytest.approx(KF_INIT_LEVEL)
    # A は field 内で相対的に高い
    assert out.loc[out["horse_id"] == "A", "kf_level_vs_field"].iloc[0] > 0
    for col in KF_FEATURE_COLS:
