"""JRDB オッズデータ（OZ/OW/OU/OT/OV）パーサ — 券種別に全組合せの**実市場オッズ**を展開。

各レコードはレース単位で、券種ごとに全組合せ（馬連153/ワイド153/馬単306/3連複816/
3連単4896）の**実際の市場オッズ**（pari-mutuel プールの倍率。控除内包）を固定長で並べる。
※旧コメントは「基準オッズ＝JRDB フェアバリュー」としていたが誤り。単勝の Σ(1/odds) overround が
≈1.35（>1.0）で控除を内包＝実市場オッズと実測確定（1.25 の締切値より高い＝前売り/前日の早い
時点）。JRDB のフェア値は別物で KYI の `kijun_odds`（馬ごと単値）の方。
列展開は非現実的（3連単4896列）なので **long 形式**（race_id, bet, combo, odds）で返す。
raw_jrdb_* には保存せず、HJC（実払戻）や連系 ΔR² 検証が直接読む。
...
def favorites(odds_long: pd.DataFrame, *, top: int = 1) -> pd.DataFrame:
    """race_id×bet ごとに市場オッズ最小（＝前売り最人気）の上位 top 組を返す。
