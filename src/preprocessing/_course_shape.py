"""コース形状マスタの読込と results への付与（純関数 + load）。

course_master.csv（scripts/scrape_course_master.py が JRA 公式から生成）を読み、
現レースの (開催, race_type) に左結合して course_* 属性列を付与する。学習(DataMerger)・
ライブ(ShutubaDataMerger)とも同じ CSV を参照し特徴量パリティを保つ。

幾何（直線/高低差/一周/幅員/回り）は物理シミュレーションの環境パラメータに、
プロファイル（脚質バイアス/馬場傾向/芝種）は出走馬×コース相性評価に用いる。
"""
...
def add_course_shape_features(results: pd.DataFrame, course_master: pd.DataFrame) -> pd.DataFrame:
    """results に course_<attr> 属性列を付与して返す（他 add_* factor と同じ純関数流儀）。
    ...
    needed = {"開催", "race_type"}
    if course_master is None or course_master.empty or not needed.issubset(out.columns):
        for c in feat_cols:
            out[c] = float("nan")
        return out
