...
def test_datamerger_merge_horse_trueskill_adds_columns():
    ...
    obj._merged_data = md
    obj.horse_trueskill_snapshot = {}
    obj._merge_horse_trueskill()
    for col in TS_FEATURE_COLS:
        assert col in obj._merged_data.columns
    assert set(obj.horse_trueskill_snapshot) == {"A", "B", "C"}


def test_shutuba_merger_live_trueskill_from_snapshot(tmp_path, monkeypatch):
    import json
    ...
    snap = {
        "A": {"mu": 30.0, "sigma": 4.0, "n_races": 20, "last_date": "2020-01-01"},
        "B": {"mu": 20.0, "sigma": 4.0, "n_races": 20, "last_date": "2020-01-01"},
    }
    snap_path = tmp_path / "horse_trueskill.json"
    snap_path.write_text(json.dumps(snap))
    monkeypatch.setattr(_local_paths.LocalPaths, "HORSE_TRUESKILL_PATH", str(snap_path))
    ...
    assert out.loc[out["horse_id"] == "A", "ts_mu"].iloc[0] == pytest.approx(30.0)
    # 未知の馬 C は初期 μ/σ
    assert out.loc[out["horse_id"] == "C", "ts_mu"].iloc[0] == pytest.approx(TS_MU)
    assert out.loc[out["horse_id"] == "C", "ts_sigma"].iloc[0] == pytest.approx(TS_SIGMA)
    # 保守的スキルの field 相対: A（強い・低 σ）は正
    assert out.loc[out["horse_id"] == "A", "ts_vs_field"].iloc[0] > 0
