...
⚠ 2026-06-29 訂正: かつて「本物のエッジ」とされた回収率（~1.9–2.1 や 0.86→3.72）は
目的変数リーク `rank_win` の産物だった（§3-42）。リーク除去後の honest 回収率は ~0.74＝
負けで、エッジは確認されていない。本スクリプトは「賭け方（資金配分）」の比較ツールであり、
エッジが存在する前提でのみ意味を持つ。現状はエッジが無いため、運用判断には使わないこと。

本スクリプトは、（エッジが存在する場合に）それを「賭け方（資金配分）」
...
def _print_by_year(groups, settle_fn, flat_unit: float):
    ...
    print("  " + "-" * 43)
    # 判定は実数に基づく（誤って「本物のエッジ」と出さない）。全年>1 のときのみ肯定。
